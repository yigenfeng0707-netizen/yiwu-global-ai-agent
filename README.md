...
> 参赛赛道：2026"直通乌镇"全球互联网大赛 OPC特色赛（One Person Company）

## 项目简介

义乌小商品出海智能体-OPC 是一款面向跨境电商的AI智能服务平台，依托义乌小商品城7.5万商户、210万+SKU的产业资源，集成7大AI Agent（市场洞察、智能选品、供应链匹配、内容生成、合规查询、智能客服、政策复制），为中小企业提供从市场分析到商品出海的全链路AI服务，助力义乌发展经验向全国39个市场采购贸易试点城市复制推广。

## OPC叙事：1个人 + 7个AI数字员工 = 一家完整的外贸公司

在 OPC（One Person Company）模式下，**一个义乌小卖家就是一家公司**：

| AI数字员工 | 替代的传统岗位 | 年人力成本对比 |
|-----------|--------------|--------------|
| 📊 市场洞察分析师 | 市场研究专员 | ¥15万 → AI订阅费 |
| 🎯 选品师 | 采购/选品经理 | ¥20万 → AI订阅费 |
| 🔗 供应链专员 | 采购跟单员 | ¥10万 → AI订阅费 |
| ✍️ 跨境文案（8语种） | 外语运营团队 | ¥40万+ → AI订阅费 |
| 🛡️ 合规顾问 | 外贸单证/法务 | ¥15万 → AI订阅费 |
| 💬 多语客服（7×24） | 客服团队 | ¥20万 → AI订阅费 |
| 🏛️ 政策顾问 | 政策申报专员 | ¥12万 → AI订阅费 |

传统外贸团队需要 7 个岗位、百万元以上年人力成本；OPC 模式下，一名创业者用 7 个 AI Agent 即可完整运营跨境贸易业务——这正是 AI 时代"超级个体"的样本。
...
- **OPC模式创新** — One Person Company：7个AI Agent组成数字团队，让一个人具备一家外贸公司的完整能力，义乌经验可复制、可推广、可AI化
...
| ✍️ Content Generation | 内容生成 | 多语言营销内容 | 8语言4平台、SEO关键词、社媒文案、广告素材 |
| 🛡️ Compliance | 合规查询 | 合规与关税查询 | 5国合规要求、1039通关、RCEP优惠、关税计算 |
| 💬 Customer Service | 智能客服 | 多语言智能客服 | 7×24小时、情绪识别、多轮对话、FAQ自动应答 |
| 🏛️ Policy Replication | 政策复制 | 1039政策推广 | 39城试点信息、政策解读、红利计算、成功案例 |

> **引擎性质（诚实标注）**：7 个 Agent 均为 **LLM 增强型**（确定性骨架/规则公式 + LLM 推理增强）；未配置 `LLM_API_KEY` 时优雅降级为纯骨架输出（响应含 `ai_used=false`）。可通过 `GET /api/v1/agents/info` 查看各 Agent 引擎类型（`engine=llm-enhanced`）。

## 义乌独有壁垒

| 壁垒 | 说明 |
|------|------|
| 📊 **义乌指数** | 全球小商品价格风向标，已接入义乌指数官网官方发布值（ywindex.com，千点基准·定期更新），为选品提供数据支撑 |
...
**① 真实接入（带时间戳，可现场自证请求-响应）**
1. **实时汇率** — `open.er-api.com` 每日参考汇率（USD/CNY/EUR 等），响应自带官方更新时间戳
2. **义乌指数官方发布值** — `ywindex.com/report` 官网公开发布的品类指数（千点基准·定期更新，非实时面板）

**② 演示静态数据（内置数据集，明确标注非实时）**
3. **义乌小商品城** — 7.5万商户、210万+SKU 内置演示数据集（规划对接官方数据库）
4. **义新欧班列** — 19条线路、50国160城物流数据（内置演示）
5. **行业报告** — 行业规模/增速内置演示数据（义乌指数子字段已改由真实源提供）

**③ 规划中（当前为演示占位，非真实平台数据）**
6. **Amazon** — 全球平台销售数据（接入规划中）
7. **Alibaba.com** — 国际站B2B数据（接入规划中）

> 数据源三态状态可通过 `GET /api/v1/data-sources` 查看；`POST /api/v1/data-sources/refresh` 可现场触发真实源刷新（答辩演示用）。
