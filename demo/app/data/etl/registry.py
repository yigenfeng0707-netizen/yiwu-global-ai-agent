...
LEGACY_SOURCE_STATUS: List[Dict[str, str]] = [
    {"name": "义乌小商品城", "status": DataSourceStatus.DEMO,
     "note": "内置品类/商户静态演示数据，非实时抓取"},
    {"name": "义新欧班列", "status": DataSourceStatus.DEMO,
     "note": "内置线路/时效/运费静态演示数据"},
    {"name": "Amazon", "status": DataSourceStatus.PLANNED,
     "note": "平台销售数据接入规划中，当前为演示占位"},
    {"name": "Alibaba.com", "status": DataSourceStatus.PLANNED,
     "note": "国际站 B2B 数据接入规划中，当前为演示占位"},
    {"name": "行业报告", "status": DataSourceStatus.DEMO,
...
class RealDataRegistry:
    ...
    def __init__(self, sources: Optional[List[RealDataSource]] = None, db: Optional[Any] = None):
    ...
    def refresh(self, name: Optional[str] = None, force: bool = False) -> Dict[str, FetchResult]:
        """刷新指定源（name=None 则全部）。
        ...
        """
        targets = [self._sources[name]] if name and name in self._sources else list(self._sources.values())
    ...
    def get_data(self, name: str, default: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def get_meta(self, name: str) -> Dict[str, Any]:
        ...
        result = self.get(name)
        if not result:
            return {"is_real": False, "source": name, "fetched_at": 0, "source_url": "",
                    "age_seconds": None, "note": "尚无成功抓取记录"}
        d = result.to_dict()
        return {
            "is_real": d["is_real"],
            "source": name,
    ...
    def get_index_for_category(self, category: str) -> Dict[str, Any]:
        ...
        base = {
            "is_real": False, "index_value": None, "scale": "",
            "as_of": "", "category_matched": "", "change_pct": None,
            "source_url": meta.get("source_url", ""), "fetched_at_iso": meta.get("fetched_at_iso", ""),
            "excerpt": "", "note": "",
        }
    ...
    def get_exchange_rate(self, currency: str = "CNY") -> Dict[str, Any]:
        ...
        if not meta.get("is_real") or not data:
            return {"is_real": False, "currency": currency, "rate": None,
                    "as_of": "", "source_url": meta.get("source_url", ""),
                    "note": "汇率真实源暂无成功抓取记录"}
    ...
    def status(self) -> List[Dict[str, Any]]:
        ...
        for name, src in self._sources.items():
            ...
            out.append({
                ...
                "key": name,
                "status": DataSourceStatus.REAL if meta.get("is_real") else DataSourceStatus.PLANNED,
    ...
    def real_count(self) -> int:
        """当前真实接入（有成功值）的源数量。"""
        return sum(1 for name in self._sources if (self.get(name) or FetchResult(source=name, is_real=False)).is_real)
    ...
    def _warm_worker(self):
        try:
            self.refresh(force=True)
            logger.info("[ETL预热] 真实数据源刷新完成：real=%d/%d", self.real_count(), len(self._sources))
        except Exception as e:  # noqa: BLE001
